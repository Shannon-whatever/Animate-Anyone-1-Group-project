...
def resize_image_keep_exact(input_image, resolution):
    H, W, C = input_image.shape
    k = float(resolution) / min(H, W)
    H_new = int(round(H * k))
    W_new = int(round(W * k))
    img = cv2.resize(input_image, (W_new, H_new), interpolation=cv2.INTER_LANCZOS4 if k > 1 else cv2.INTER_AREA)
    return img
...
class DWposeDetector:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        input_image = HWC3(input_image)
        input_image = resize_image_keep_exact(input_image, detect_resolution)
        H, W, C = input_image.shape
        with torch.no_grad():
            candidate, subset = self.pose_estimation(input_image)
            nums, keys, locs = candidate.shape
